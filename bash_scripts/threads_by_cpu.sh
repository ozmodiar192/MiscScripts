#!/bin/bash
if ps -eo pcpu,pid,args | sort -k 1 -r | tr -s " " | head -n 2 | sed '1d' | grep -q java;
	then
...
<<<<<<< HEAD
		ps -eLo pid,lwp,nlwp,ruser,pcpu,stime,etime | grep `ps -eo pcpu,pid | sort -k 1 -r | tr -s " " | sed 's/^ *//' | head -n 2 | sed '1d' | cut -d " " -f 2` | tr -s " " | sort -k 5 -r | awk '{print $2, $5}' | grep -v " 0.0"
		...
		topthreads=( $(ps -eLo pid,lwp,nlwp,ruser,pcpu,stime,etime | grep `ps -eo pcpu,pid | sort -k 1 -r | sed 's/^ *//' | tr -s " " | head -n 2 | sed '1d' | cut -d " " -f 2` | tr -s " " | sort -k 5 -r | grep -v " 0.0"| awk '{print $2}' ) )
=======
		ps -eLo pid,lwp,nlwp,ruser,pcpu,stime,etime | grep `ps -eo pcpu,pid | sort -n -k 1 -r | tr -s " " | sed 's/^ *//' | head -n 2 | sed '1d' | cut -d " " -f 2` | tr -s " " | sort -n -k 5 -r | awk '{print $2, $5}' | grep -v " 0.0"
		printf "\n\nHere are the converted Hex IDs of the threads using CPU, sorted by cpu utilization. Find them in the thread dump below as \"nid\" to identify problematic processes: \n"
		#create an array of thread IDs in decimal
...
>>>>>>> fc7aa4fed6aae32e315d41f0da5bb981b4c184ae
		...
		javapid=$(ps -eo pcpu,pid | sort -k 1 -r | tr -s " " | sed 's/^ *//' | head -n 2 | sed '1d' | cut -d " " -f 2)
		...
		echo "top utilization process isn't java"
fi
